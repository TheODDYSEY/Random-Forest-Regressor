...
st.sidebar.markdown(' Author : TheODDYSEY </h5>', unsafe_allow_html=True)
st.sidebar.markdown('Select One output and at least one input Variable</h1>', unsafe_allow_html=True)
...
st.markdown(' This program allows to build a random forest model and to implement it to allow the user to predict an output target variable from given values of input variables. To build the model, the user select a combination of one "output" variable and at least one input variable from the list. </h4>', unsafe_allow_html=True)

# Expander for dataset information
with st.expander('About the Dataset', expanded=True):
    st.write('''
        - **Data Source**: The dataset used in this project is a hypothetical dataset named "Data_BarbeauFlux_noNaN.csv".
        - **Data Description**: 
            - The dataset contains various columns representing different spectral reflectance bands (e.g., R_450, R_550, R_650, R_720, R_750, R_800).
            - Each column represents reflectance values at specific wavelengths in nanometers.
            - The target variable (output) could be any column selected by the user.
        - **Application**: This dataset is used to predict one spectral reflectance band based on the others using a Random Forest model.
    ''')
...
ax.text(0.05, 0.9, text, transform=ax.transAxes, color='white', fontsize=12)

# Customize title and labels
scatter_plot.set_title(f'Predicted vs Observed {output_variable_model}', color='white')
scatter_plot.set_xlabel('Observed', color='white')
scatter_plot.set_ylabel('Predicted', color='white')
...
st.markdown(' Use Sidebar menu to select the values of input variables to predict the target variable. </h4>', unsafe_allow_html=True)

# User input for feature values
st.sidebar.markdown(' Select the values of input variables to predict the target variable</h2>', unsafe_allow_html=True)
